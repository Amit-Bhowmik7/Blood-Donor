...
public class Admin_Volunteer_Post_Delete_Dialog extends AppCompatActivity {
    private AppCompatButton cancelButton, deleteButton;
    private ImageButton backButton, homeButton;
    private DatabaseReference volunteerRequestPostDatabase;
    private static final String PREFERENCES = "volunteerRequest";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin_volunteer_post_delete_dialog);

        // Initialize Firebase Database
        volunteerRequestPostDatabase = FirebaseDatabase.getInstance().getReference("VolunteerList");
    ...
    private void backButton() {
        Intent intent = new Intent(getApplicationContext(), Admin_Volunter_Post_Edit.class);
        startActivity(intent);
    }
    ...
    private void deletePost() {
        ...
        String mobileNumber = volunteerRequestPost.getString("v_mobileNumber", null);

        if (mobileNumber != null) {
            // Remove user data from Firebase
            volunteerRequestPostDatabase.child(mobileNumber).removeValue().addOnCompleteListener(task -> {
    ...
    private void cancelButton() {
        Intent intent = new Intent(getApplicationContext(), Admin_Volunteer_List.class);
        startActivity(intent);
        finish();
    }
}