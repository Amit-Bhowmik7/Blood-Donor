...
public class Admin_Users_account_Delete_Dialog extends AppCompatActivity {
    private AppCompatButton cancelButton, deleteButton;
    private ImageButton backButton, homeButton;
    private DatabaseReference userRequestPostDatabase;
    private static final String PREFERENCES = "UserPrefs";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin_users_account_delete_dialog);

        // Initialize Firebase Database
        userRequestPostDatabase = FirebaseDatabase.getInstance().getReference("UsersList");
    ...
    private void backButton() {
        Intent intent = new Intent(getApplicationContext(), Admin_Users_List.class);
        startActivity(intent);
    }
    ...
    private void deletePost() {
        ...
        String mobileNumber = volunteerRequestPost.getString("mobileNumber", null);

        if (mobileNumber != null) {
            // Remove user data from Firebase
            userRequestPostDatabase.child(mobileNumber).removeValue().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(getApplicationContext(), "User account delete successfully", Toast.LENGTH_SHORT).show();
